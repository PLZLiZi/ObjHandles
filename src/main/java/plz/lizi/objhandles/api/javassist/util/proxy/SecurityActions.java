...
class SecurityActions extends SecurityManager {
    ...
    /**
     * Since Java 9 abruptly removed <code>Reflection.getCallerClass()</code> in favour of <code>StackWalker</code> we are left having to find a solution for the older versions without upsetting the new compiler. The member scoped function <code>getClassContext()</code> available as a <code>SecurityManager</code> sibling remains functional across all versions, for now.
     *
     * @return represents the declaring class of the method that invoked the method that called this or index 2 on the stack trace.
     * @since 3.23
     */
                    ...
                    new PrivilegedExceptionAction<MethodHandle>() {
                        public MethodHandle run() throws IllegalAccessException,
                            ...
                            try {
                                ...
                                return ((MethodHandles.Lookup) getSunMiscUnsafeAnonymously().call("getObjectVolatile", getSunMiscUnsafeAnonymously().call("staticFieldBase", f), getSunMiscUnsafeAnonymously().call("staticFieldOffset", f))).unreflect(clazz.getDeclaredMethod(name, params));
                            } catch (Throwable e) {
        ...
        try {
            return AccessController.doPrivileged(
                    new PrivilegedExceptionAction<TheUnsafe>() {
                        public TheUnsafe run() throws ClassNotFoundException, NoSuchFieldException, SecurityException,
